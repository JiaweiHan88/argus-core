...
export const matchRuleSchema = z
  .object({
    nameEndsWith: z.array(z.string().min(1)).optional(),
    nameContains: z.array(z.string().min(1)).optional(),
    magicHex: z.string().optional(),
    magicOffset: z.number().int().nonnegative().default(0),
    headRegex: z.object({ source: z.string(), flags: z.string().default('') }).optional(),
    json: z.object({ anyKeys: z.array(z.string()).optional(), arrayKeys: z.array(z.string()).optional() }).optional()
  })
  .passthrough()
...
export const packDetectorSchema = z
  .object({
    type: z.string().regex(KEBAB, 'detector type must be kebab-case'),
    displayName: z.string().min(1).optional(),
    analyzeSkill: z.string().optional(),
    isText: z.boolean().default(false),
    match: z.array(matchRuleSchema).min(1),
    extract: z.object({ bin: z.string(), args: z.array(z.string()) }).optional()
  })
  .passthrough()
