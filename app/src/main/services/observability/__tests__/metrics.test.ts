...
function seed(db: ReturnType<typeof openDb>): void {
  ...
  db.prepare(
    `INSERT INTO cases (id, slug, title, status, tags, created_at, updated_at) VALUES (1,'c1','C1','closed','[]',?,?)`
  ).run(now, now)
  db.prepare(
    `INSERT INTO turns (case_id, session_id, turn_index, status, input_tokens, output_tokens, cost_usd, duration_ms, model, created_at)
              VALUES (1,1,0,'success',100,50,0.02,1200,'claude-opus-4-8',?)`
  ).run(now)
  db.prepare(
    `INSERT INTO turns (case_id, session_id, turn_index, status, input_tokens, output_tokens, cost_usd, duration_ms, model, created_at)
              VALUES (1,1,1,'error',NULL,NULL,NULL,NULL,NULL,?)`
  ).run(now)
  db.prepare(
    `INSERT INTO tool_calls (case_id, session_id, turn_id, tool, args_hash, risk, decision, duration_ms, created_at)
              VALUES (1,1,1,'Bash','h','MEDIUM','user',5,?)`
  ).run(now)
  db.prepare(
    `INSERT INTO tool_calls (case_id, session_id, turn_id, tool, args_hash, risk, decision, duration_ms, created_at)
              VALUES (1,1,1,'Bash','h','MEDIUM','denied',5,?)`
  ).run(now)
  db.prepare(
    `INSERT INTO findings (case_id, session_id, turn_id, summary, review_state, created_at) VALUES (1,1,1,'F1','accepted',?)`
  ).run(now)
  db.prepare(
    `INSERT INTO findings (case_id, session_id, turn_id, summary, review_state, created_at) VALUES (1,1,1,'F2','pending',?)`
  ).run(now)
}
...
describe('metrics aggregation', () => {
  it('computes global metrics with NULL cost treated as unknown', () => {
    ...
    expect(m.byModel).toEqual([
      { model: 'claude-opus-4-8', inputTokens: 100, outputTokens: 50, costUsd: 0.02 }
    ])
