export interface SynthesizedAcpRequest {
  name: string
...
export function synthesizeAcpPermission(
  ...
  switch (kind) {
    case 'execute':
      return { name: 'shell', input: { command: args.command ?? args.fullCommandText } }
    case 'read':
      return { name: 'read', input: { file_path: args.path ?? args.file_path } }
    case 'edit':
    case 'delete':
    case 'move':
      return { name: 'write', input: { file_path: args.path ?? args.file_path } }
    case 'fetch':
      return { name: 'fetch', input: { url: args.url } }
    default:
