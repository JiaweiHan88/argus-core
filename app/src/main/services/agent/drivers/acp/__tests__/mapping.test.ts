...
it('maps ACP permission kinds to canonical tool + input', () => {
  expect(synthesizeAcpPermission('execute', { command: 'rm -rf /' })).toEqual({
    name: 'shell',
    input: { command: 'rm -rf /' }
  })
  expect(synthesizeAcpPermission('read', { path: '/a' })).toEqual({
    name: 'read',
    input: { file_path: '/a' }
  })
  expect(synthesizeAcpPermission('edit', { path: '/a' })).toEqual({
    name: 'write',
    input: { file_path: '/a' }
  })
})
