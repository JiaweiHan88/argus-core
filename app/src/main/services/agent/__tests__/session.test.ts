...
describe('CaseSession', () => {
  it('send() enqueues an SDK user message and emits turn.started', async () => {
    ...
    const first = (await iter.next()).value as { type: string; message: { content: [{ text: string }] } }
  ...
  it('normalizes streamed messages and persists the resume cursor + turn usage', async () => {
    ...
    const s = makeSession(sdk)
    s.send('go')
    sdk.messages.push({ type: 'system', subtype: 'init', session_id: '11111111-1111-4111-8111-111111111111', model: 'm' })
    sdk.messages.push({
      type: 'stream_event', session_id: 'x',
      event: { type: 'content_block_delta', delta: { type: 'text_delta', text: 'hi' } }
    })
    sdk.messages.push({
      type: 'result', subtype: 'success', session_id: '11111111-1111-4111-8111-111111111111',
      usage: { input_tokens: 5, output_tokens: 2 }, total_cost_usd: 0.001, duration_ms: 10, is_error: false
    })
    await flush()
    ...
    const sess = db.prepare(`SELECT sdk_session_id, turn_count FROM sessions`).get() as {
      sdk_session_id: string; turn_count: number
    }
    expect(sess.sdk_session_id).toBe('11111111-1111-4111-8111-111111111111')
    expect(sess.turn_count).toBe(1)
    const turn = db.prepare(`SELECT status, input_tokens FROM turns`).get() as { status: string; input_tokens: number }
  ...
  it('canUseTool: LOW auto-allows and logs; HIGH round-trips an approval', async () => {
    ...
    const canUseTool = sdk.captured.options!.canUseTool as (
      n: string, i: Record<string, unknown>, o: { signal: AbortSignal }
    ) => Promise<{ behavior: string; message?: string }>

    const low = await canUseTool('Bash', { command: 'git log' }, { signal: new AbortController().signal })
    expect(low.behavior).toBe('allow')

    const highP = canUseTool('Bash', { command: 'git push' }, { signal: new AbortController().signal })
    await flush()
    const opened = events.find((e) => e.type === 'request.opened')!
    expect(opened.payload).toMatchObject({ tool: 'Bash', risk: 'HIGH' })
    expect(
      s.respond({ requestId: (opened.payload as { requestId: string }).requestId, kind: 'deny', comment: 'no' })
    ).toBe(true)
    ...
    const rows = db.prepare(`SELECT tool, risk, decision FROM tool_calls ORDER BY id`).all() as {
      tool: string; risk: string; decision: string
    }[]
    expect(rows).toEqual([
  ...
  it('allow-session creates a grant that short-circuits the next ask', async () => {
    ...
    const p1 = canUseTool('Bash', { command: 'git fetch origin' }, { signal: new AbortController().signal })
    await flush()
    const opened = events.find((e) => e.type === 'request.opened')!
    s.respond({ requestId: (opened.payload as { requestId: string }).requestId, kind: 'allow-session' })
    expect((await p1).behavior).toBe('allow')

    const p2 = await canUseTool('Bash', { command: 'git fetch origin' }, { signal: new AbortController().signal })
    expect(p2.behavior).toBe('allow')
    const last = db.prepare(`SELECT decision FROM tool_calls ORDER BY id DESC LIMIT 1`).get() as { decision: string }
  ...
  it('stop() drains pending approvals with request.resolved cancelled', async () => {
    ...
    const pending = canUseTool('Bash', { command: 'git push' }, { signal: new AbortController().signal })
    await flush()
    await s.stop('stopped')
    expect((await pending).behavior).toBe('deny')
    expect(
      events.some((e) => e.type === 'request.resolved' && (e.payload as { decision: string }).decision === 'cancelled')
    ).toBe(true)
