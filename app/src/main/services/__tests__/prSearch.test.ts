...
const linkRepo = (slug: string, remote: string | null, p = '/tmp/mapbox-sdk'): void => {
...
describe('searchPrsForCase', () => {
  it('searches the linked repo and classifies the result', async () => {
    linkRepo('c1', 'git@github.com:mapbox/mapbox-sdk.git')
    let seen: string[] = []
    ...
    expect(r.searchedRepos).toEqual(['mapbox/mapbox-sdk'])
    expect(seen).toContain('NN-5165')
    expect(seen).toContain('--repo')
    expect(seen).toContain('mapbox/mapbox-sdk')
  ...
  it('passes one --repo per linked GitHub repo in a single invocation', async () => {
    db.prepare(`UPDATE cases SET workspaces = ? WHERE slug = ?`).run(
      JSON.stringify([
        { path: '/tmp/a', remote: 'git@github.com:mapbox/a.git', branch: 'main' },
        { path: '/tmp/b', remote: 'https://github.com/mapbox/b.git', branch: 'main' }
      ]),
      'c1'
    )
    let calls = 0
  ...
  it('filters out non-GitHub remotes and reports no searchable repo', async () => {
    linkRepo('c1', 'git@gitlab.com:mapbox/x.git')
    let called = false
    const gh: Runner = async () => {
      called = true
      return GH_JSON
