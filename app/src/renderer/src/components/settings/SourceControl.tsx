      ...
      <div className="flex items-center gap-3 px-3 py-2">
        ...
        {status ? (
          <>
            ...
            {status.authenticated ? (
              <span className="flex min-w-0 flex-wrap items-center gap-x-1 text-sm text-dim">
                <span>Authenticated as</span>
                <RedactedText
                  value={status.login ?? ''}
                  aria-label="Toggle GitHub login visibility"
                />
              </span>
            ) : (
              <span className="text-sm text-dim">
                {status.installed ? `${status.detail} — run \`gh auth login\`` : status.detail}
              </span>
            )}
          </>
        ) : (
